...
const UsersPage = () => {
  ...
  const { hasRole, user: currentUser, isSuperAdmin } = useAuth();
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Filter out core team roles from the user list (they're internal only)
      ...
      const usersWithRoles = (profiles || [])
        ...
        // Filter out core team members from the customer admin view
        .filter((user) => !coreRoles.includes(user.role || ""));
        ...
        <div className="flex gap-4 flex-1">
          ...
          <Select value={roleFilter} onValueChange={setRoleFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="admin">Admin / Business Manager</SelectItem>
          ...
          <div className="py-4">
            ...
            {selectedUser?.role === "super_admin" && !isSuperAdmin && (
              <p className="text-sm text-warning mt-2">
                ⚠️ Cannot modify Super Admin role. Only Company Owner can change this.
              </p>
            )}
          </div>
          <DialogFooter>
