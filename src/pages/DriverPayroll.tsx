...
// Nigeria Personal Income Tax - New Law (2024) Brackets
// Based on https://fiscalreforms.ng/index.php/pit-calculator/
// Minimum wage threshold: ₦70,000/month = ₦840,000/year (exempt)
interface TaxDeductibles {
  nhf: number;      // National Housing Fund (2.5% of basic)
  nhis: number;     // National Health Insurance
  pension: number;  // Pension contribution (8% employee)
  lifeInsurance: number;
  rentRelief: number;
}

const calculateTaxableIncome = (grossIncome: number, deductibles?: TaxDeductibles): number => {
  if (!deductibles) return grossIncome;
...
const calculateNigeriaTax = (
  annualIncome: number, 
  deductibles?: TaxDeductibles
): { tax: number; effectiveRate: number; brackets: { range: string; rate: number; taxable: number; tax: number }[]; taxableIncome: number; totalDeductions: number } => {
  // Calculate taxable income after deductions
  const totalDeductions = deductibles ? 
    (deductibles.nhf || 0) + (deductibles.nhis || 0) + (deductibles.pension || 0) + 
    (deductibles.lifeInsurance || 0) + (deductibles.rentRelief || 0) : 0;
  
  const taxableIncome = calculateTaxableIncome(annualIncome, deductibles);
  
  // Minimum wage exemption (₦70,000/month = ₦840,000/year)
  const MINIMUM_WAGE_THRESHOLD = 840000;
  
  // If income is at or below minimum wage, no tax
  if (taxableIncome <= MINIMUM_WAGE_THRESHOLD) {
    return { tax: 0, effectiveRate: 0, brackets: [], taxableIncome, totalDeductions };
  }
  ...
  return { tax, effectiveRate, brackets: bracketDetails, taxableIncome, totalDeductions };
};
...
const DriverPayrollPage = () => {
  ...
  const generatePayslipPDF = (payroll: PayrollSummary) => {
    ...
    doc.setFontSize(10);
    doc.text("RouteAce Logistics", 15, 50);
